...
async function connect(proxyData) {
    ...
    // Không còn kiểm tra IP để xác minh kết nối - báo connected ngay lập tức
...
async function checkIP() {
    await chrome.storage.local.set({ ipCheckStatus: "checking" });
    const ip = await getIP();
    if (ip) {
        await chrome.storage.local.set({ ip: ip, ipCheckStatus: "done" });
    } else {
        await chrome.storage.local.set({ ip: "Unreachable", ipCheckStatus: "done" });
    }
}
