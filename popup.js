...
async function updateUI() {
    const data = await chrome.storage.local.get(["connected", "proxy", "ip", "status", "ipCheckStatus"]);
    ...
    const btnCheckIP = document.getElementById("checkIP");
    ...
    if (data.connected) {
        ...
        ipInfo.innerText = data.ipCheckStatus === "checking" ? "Checking..." : (data.ip || "Not checked");
        
        btnConnect.style.display = "none";
        btnConnect.disabled = false;
        btnDisconnect.style.display = "flex";
        btnCheckIP.style.display = "flex";
...
document.getElementById("checkIP").onclick = () => {
    chrome.runtime.sendMessage({ action: "checkIP" });
};
